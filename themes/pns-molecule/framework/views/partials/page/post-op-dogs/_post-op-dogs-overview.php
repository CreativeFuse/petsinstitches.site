...
$categoryOverview = category_description(184);
$anchorMenu[0] = array(
  'title' => 'Overview',
  'link' => '#overview'
);
$postIds = array_keys($postopcare[184]['records']);
foreach ($postopcare[184]['records'] as $recordId => $record) {
                      ...
                      if( get_row_layout() == 'general_care' ){
                        if( have_rows('general_care_block') ){
                          while ( have_rows('general_care_block') ) : the_row();
                            Molecule_Router::render( 'page/post-op-dogs', '_post-op-dogs', 'section' );
                          endwhile;
                        }
                          ...
                          <div class="c-card__feed">
                            <?php
                              ...
                              foreach ($related_posts as $post) {
                                // Each item rendered uses the following component
                                Molecule_Router::render( 'page/post-op-dogs', '_post-op-dogs', 'blog-posts', $post );
                              }
                            ?>
                          </div>
                        </div>
